...
//  Modified by Jeff Zhong on 13-05-31
//
...
-(id)initWithFrameRect:(CGRect)rect ImageArray:(NSArray *)imgArr TitleArray:(NSArray *)titArr
{
	...
	if ((self=[super initWithFrame:rect])) {
        ...
        for (int i=0; i<pageCount; i++) {
            NSString *imgURI=[imageArray objectAtIndex:i];
            UIImageView *imgView=[[[UIImageView alloc] init] autorelease];
            if ([imgURI hasPrefix:@"http://"]) {
                //网络图片 请使用ego异步图片库
                //[imgView setImageWithURL:[NSURL URLWithString:imgURL]];
                NSLog(@"Load online AD image: %@", imgURI);
                [imgView setImageWithURL:[NSURL URLWithString:imgURI] placeholderImage:[UIImage imageNamed:@"adPlaceHolder.png"]];
            }
            else
            {
                UIImage *img=[UIImage imageNamed:imgURI];
